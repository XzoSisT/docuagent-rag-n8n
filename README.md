...
DocuAgent is a small, explicit Retrieval-Augmented Generation (RAG) learning project. A FastAPI backend extracts text from PDF files, creates embeddings, stores document chunks in Qdrant, and exposes guarded semantic retrieval through a REST API. An n8n workflow retrieves context before every model call, rejects unsupported questions deterministically, and appends citations from verified metadata.
...
flowchart TD
    ...
    Q --> API
    API --> HTTP
    HTTP --> G{Relevant context?}
    G -->|No| R[Deterministic rejection]
    G -->|Yes| C[Basic LLM Chain]
    CM[Local Ollama Chat Model] --> C
    C --> S[Append verified citations]
    R --> U
    S --> U
```

The Python backend performs document ingestion and guarded retrieval only. It does not generate the final conversational answer. Retrieval is deterministic: n8n calls `/search` before every LLM invocation, and the LLM is skipped when no result passes the calibrated relevance threshold.
...
- Configurable relevance threshold and exact PDF filename filtering
- Deterministic rejection when no trustworthy context is available
- Verified citations appended from retrieval metadata rather than generated by the LLM
...
- Reproducible live retrieval evaluation and GitHub Actions CI
...
user query
  ...
  -> filter by relevance threshold and optional requested PDF filename
  -> reject when no trustworthy context remains
  -> otherwise generate an answer and append verified source/page metadata
```
...
Relevant Context Found?
     | yes                         | no
     v                             v
Basic LLM Chain              Deterministic rejection
     |
     v
...
With the Compose stack running and the documented PDFs indexed:

```powershell
py -3.11 -m uv run python scripts/evaluate_retrieval.py
```

The versioned cases in `evaluation/retrieval_cases.json` check expected sources for NIST, RAG, and ETDA questions plus deterministic rejection for out-of-scope and missing-document questions. The relevance threshold of `0.42` was calibrated from these local evaluation cases; it should be recalibrated when the corpus or embedding model changes.
...
Screenshots are optional. For a concise repository, capture only the final workflow and one successful cited response after completing the manual test:
...
- Reranking and a larger labeled evaluation set
...
- Optional hosted demo with authentication
